...
class _CreateMatchScreenState extends State<CreateMatchScreen> {
  ...
  Future<List<Team>> _getAvailableTeamsForTeam2() async {
    ...
    for (final team in _allTeams) {
      ...
      final isAvailable = await _apiService.isTeamAvailableAtTime(team.id, matchDateTime);
      if (isAvailable) {
  ...
  Future<void> _createMatch() async {
    ...
    if (_selectedTeam1Id == null || _selectedTeam1Id!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            LocalizationService().translate('team_1_required') ?? 'Please select Team 1',
          ),
    ...
    if (_selectedTeam2Id == null || _selectedTeam2Id!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            LocalizationService().translate('team_2_required') ?? 'Please select Team 2',
          ),
          backgroundColor: Colors.red,
        ),
      );
    ...
    if (_selectedTeam1Id == _selectedTeam2Id) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            LocalizationService().translate('teams_must_be_different') ?? 'Team 1 and Team 2 must be different',
          ),
          backgroundColor: Colors.red,
        ),
      );
          ...
          child: Center(
            child: SingleChildScrollView(
              ...
              padding: EdgeInsets.only(
                ...
                bottom: MediaQuery.of(context).viewInsets.bottom + ResponsiveConstants.getResponsiveSpacing(context, 'xl'),
              ),
            child: _isLoadingTeams
                ...
                : _allTeams.isEmpty
                    ? Column(
                          ...
                          Text(
                            LocalizationService().translate('no_teams_available'),
                          ...
                          Text(
                            LocalizationService().translate('create_teams_first_message'),
                                ...
                                children: [
                                          ...
                                          Container(
                                            ...
                                            child: DropdownButtonFormField<String>(
                                              ...
                                              decoration: InputDecoration(
                                                labelText: LocalizationService().translate('team_1'),
                                                border: InputBorder.none,
                                                prefixIcon: Icon(Icons.group, color: context.colors.primary),
                                                contentPadding: EdgeInsets.symmetric(
                                                  horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                              ...
                                              items: _allTeams.map((team) {
                                                return DropdownMenuItem(
                                                  value: team.id,
                                                  child: Text(team.name, style: Theme.of(context).textTheme.bodyMedium),
                                                );
                                              }).toList(),
                                              onChanged: (value) => setState(() => _selectedTeam1Id = value),
                                              validator: (value) {
                                                if (value == null || value.isEmpty) {
                                                  return LocalizationService().translate('team_1_required');
                                                }
                                                return null;
                                              },
                                            ),
                                      ...
                                      color: context.colors.textPrimary,
                                          ...
                                          Container(
                                            ...
                                            child: FutureBuilder<List<Team>>(
                                              future: _getAvailableTeamsForTeam2(),
                                              builder: (context, snapshot) {
                                                final availableTeams = snapshot.data ?? _allTeams;
                                                return DropdownButtonFormField<String>(
                                                  initialValue: availableTeams.any((t) => t.id == _selectedTeam2Id) ? _selectedTeam2Id : null,
                                                  decoration: InputDecoration(
                                                    labelText: LocalizationService().translate('team_2'),
                                                    border: InputBorder.none,
                                                    prefixIcon: Icon(Icons.group, color: context.colors.primary),
                                                    contentPadding: EdgeInsets.symmetric(
                                                      horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                                  ...
                                                  items: availableTeams.map((team) {
                                                    ...
                                                    return DropdownMenuItem(
                                                      value: team.id,
                                                      child: Text('${team.name} ($memberCount)', style: Theme.of(context).textTheme.bodyMedium),
                                                    );
                                                  }).toList(),
                                                  onChanged: (value) => setState(() => _selectedTeam2Id = value),
                                                  validator: (value) {
                                                    if (value == null || value.isEmpty) {
                                                      return LocalizationService().translate('team_2_required');
                                                    }
                                                    return null;
                                                  },
                                                );
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonFormField<int>(
                                      ...
                                      decoration: InputDecoration(
                                        labelText: LocalizationService().translate('number_of_players_required'),
                                        border: InputBorder.none,
                                        prefixIcon: Icon(Icons.people, color: context.colors.primary),
                                        contentPadding: EdgeInsets.symmetric(
                                          horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                          vertical: ResponsiveConstants.getResponsiveSpacing(context, 'md'),
                                        ),
                                      ),
                                      items: [10, 11, 15, 18, 22, 25, 30].map((players) {
                                        return DropdownMenuItem(
                                          value: players,
                                          child: Text('$players ${LocalizationService().translate('players')}', style: Theme.of(context).textTheme.bodyMedium),
                                        );
                                      }).toList(),
                                      onChanged: (value) => setState(() => _totalPlayers = value ?? 22),
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonFormField<String>(
                                      ...
                                      decoration: InputDecoration(
                                        labelText: LocalizationService().translate('match_type'),
                                        border: InputBorder.none,
                                        prefixIcon: Icon(Icons.group_work, color: context.colors.primary),
                                        contentPadding: EdgeInsets.symmetric(
                                          horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                          vertical: ResponsiveConstants.getResponsiveSpacing(context, 'md'),
                                        ),
                                      ...
                                      items: [
                                        DropdownMenuItem(value: 'mixed', child: Text(LocalizationService().translate('mixed'), style: Theme.of(context).textTheme.bodyMedium)),
                                        DropdownMenuItem(value: 'male', child: Text(LocalizationService().translate('male'), style: Theme.of(context).textTheme.bodyMedium)),
                                        DropdownMenuItem(value: 'female', child: Text(LocalizationService().translate('female'), style: Theme.of(context).textTheme.bodyMedium)),
                                      ],
                                      onChanged: (value) => setState(() => _selectedMatchType = value ?? 'mixed'),
                                      validator: (value) {
                                        if (value == null || value.isEmpty) {
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonFormField<bool>(
                                      ...
                                      items: const [
                                        DropdownMenuItem(value: false, child: Text('One-time')),
                                        DropdownMenuItem(value: true, child: Text('Recurring')),
                                      ],
                                    ...
                                    Container(
                                      ...
                                      child: DropdownButtonFormField<String>(
                                        ...
                                        decoration: InputDecoration(
                                          labelText: 'Pattern',
                                          border: InputBorder.none,
                                          prefixIcon: Icon(Icons.calendar_month, color: context.colors.primary),
                                          contentPadding: EdgeInsets.symmetric(
                                            horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                            vertical: ResponsiveConstants.getResponsiveSpacing(context, 'md'),
                                          ),
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButtonFormField<int>(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        border: InputBorder.none,
                                        prefixIcon: Icon(Icons.timer, color: context.colors.primary),
                                        contentPadding: EdgeInsets.symmetric(
                                          horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                          vertical: ResponsiveConstants.getResponsiveSpacing(context, 'md'),
                                        ),
                                      ),
                                      items: [45, 60, 90, 120].map((minutes) {
                                        return DropdownMenuItem(value: minutes, child: Text('$minutes min'));
                                      }).toList(),
                                        ...
                                        child: InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            child: InputDecorator(
                                              decoration: InputDecoration(
                                                labelText: LocalizationService().translate('match_date'),
                                              ...
                                              child: Text('${_selectedDate.day}/${_selectedDate.month}/${_selectedDate.year}', style: Theme.of(context).textTheme.bodyMedium),
                                            ),
                                          ),
                                        ),
                                      ),
                                      ...
                                      Expanded(
                                        child: InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            child: InputDecorator(
                                              decoration: InputDecoration(
                                                labelText: LocalizationService().translate('match_time'),
                                                prefixIcon: Icon(Icons.access_time, color: context.colors.primary),
                                                border: InputBorder.none,
                                                contentPadding: EdgeInsets.symmetric(
                                                  horizontal: ResponsiveConstants.getResponsiveSpacing(context, 'lg'),
                                                  vertical: ResponsiveConstants.getResponsiveSpacing(context, 'md'),
                                              ...
                                              child: Text(_selectedTime.format(context), style: Theme.of(context).textTheme.bodyMedium),
                                            ),
                                          ),
                                        ),
                                      ),
