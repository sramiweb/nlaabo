...
CREATE TABLE public.users (
    ...
    name TEXT NOT NULL,
    email TEXT NOT NULL, -- Removed UNIQUE constraint since auth.users handles email uniqueness
...
-- Allow authenticated users to insert their own profile (for manual profile creation)
CREATE POLICY "Users can create own profile" ON public.users
    FOR INSERT WITH CHECK (auth.uid() = id);

-- Allow the trigger function to create/update profiles (needed for automatic profile creation)
